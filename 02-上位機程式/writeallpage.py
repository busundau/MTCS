...
class SerialGUI:
    def __init__(self, root):
        ...
        ttk.Combobox(root, textvariable=self.baud_var,
                     values=["9600","19200","38400","57600","115200"], width=10).grid(row=1, column=1)
    ...
    def send_loop(self):
        while self.running:
            for hex_str in self.hex_cmds:
                ...
                try:
                    self.ser.write(data)
                    self.log.insert(tk.END, f"Sent: {hex_str}\n")
                    time.sleep(self.delay)  # 等待 MCU 回傳資料
                    if self.ser.in_waiting:
                        response = self.ser.read(self.ser.in_waiting)
                        ascii_text = ''.join([chr(b) if 32 <= b <= 126 else '.' for b in response])
                        self.log.insert(tk.END, f"Recv ASCII: {ascii_text}\n")
                except Exception as e:
                    self.log.insert(tk.END, f"Write/Read failed: {e}\n")
                    self.running = False
                    break
                time.sleep(self.delay)

        self.log.insert(tk.END, "End of send loop.\n")
