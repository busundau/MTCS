...
/* #define DISABLE_UART */
    ...
    #endif
#endif
#define  jacob_debug 1
#ifdef jacob_debug
#define DEBUG_PORT   UART1 //UART1:起子和mcu通訊用 UART0:debug和barcode用 //jacob release need uart1 modify to  uart0 20250505 
#else
#define DEBUG_PORT   UART0 //UART1:起子和mcu通訊用 UART0:debug和barcode用 //jacob release need uart1 modify to  uart0 20250505 
#endif
/*---------------------------------------------------------------------------------------------------------*/
/* Global variables                                                                                        */
/*---------------------------------------------------------------------------------------------------------*/
...
# else   // ndef DEBUG_ENABLE_SEMIHOST //2022/08/19 #加空格避開PSPad載入錯誤，可能是PSPad BUG???
...
// 	asm("MOVS    r0, #4                        \n"
  __asm("MOVS    r0, #4                        \n"	//2022/08/19
        ...
        "MRS     R0, PSP                       \n" /*; stack use PSP, read PSP */
        "MOV     R1, LR                        \n" /*; LR current value */
        "BL      Hard_Fault_Handler            \n"	//2022/08/19 改BL長距離 https://review.trustedfirmware.org/c/TF-M/trusted-firmware-m/+/5443
        "1:                                    \n"
        "MRS     R0, MSP                       \n" /*; LR current value */
        "BL      Hard_Fault_Handler            \n"	//2022/08/19 改BL長距離 https://review.trustedfirmware.org/c/TF-M/trusted-firmware-m/+/5443
